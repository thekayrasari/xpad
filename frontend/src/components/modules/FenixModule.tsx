...
const STORAGE_KEY = 'xpad-fenix-ip';
...
export const FenixModule: React.FC = () => {
    ...
    const fenixUrl = ip ? `http://${ip}:${FENIX_PORT}` : '';
    ...
    const connect = async (targetIp = ip) => {
        ...
        try {
            ...
            await fetch(`http://${targetIp}:${FENIX_PORT}`, { mode: 'no-cors', signal: ctrl.signal });
        ...
        <div className="w-full h-full font-sans text-text-primary bg-transparent flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden relative">
                ...
                {!ip && connectionState === 'idle' && (
                    <div className="flex flex-col items-center justify-center h-full gap-5 text-center px-8 max-w-md mx-auto">
                        ...
                        <div>
                            <h2 className="text-xl font-extrabold uppercase tracking-wide text-text-primary mb-2">Fenix xPad</h2>
                            <p className="text-sm font-bold text-text-secondary leading-relaxed">
                                Enter the local network IP of the PC running your flight sim above to connect to the Fenix A320/A330 xPad.
                            </p>
                        </div>
                        <button
                ...
                {connectionState === 'detecting' && (
                    <div className="flex flex-col items-center justify-center h-full gap-5 text-center px-8">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-bold text-text-secondary">{fenixUrl}</p>
                        </div>
                    </div>
                )}
                ...
                {connectionState === 'offline' && (
                    <div className="flex flex-col items-center justify-center h-full gap-5 text-center px-8 max-w-md mx-auto">
                        ...
                        <div>
                            <h2 className="text-xl font-extrabold uppercase tracking-wide text-text-primary mb-2">Fenix xPad Not Found</h2>
                            <p className="text-sm font-bold text-text-secondary leading-relaxed">
                                Could not reach <span className="text-accent-blue">{fenixUrl}</span>.
                                <br />Make sure MSFS is running with a Fenix aircraft loaded, or check the IP address.
                            </p>
                        </div>
                ...
                {connectionState === 'cert_blocked' && (
                    <div className="flex flex-col items-center justify-center h-full gap-5 text-center px-8 max-w-xl mx-auto">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-bold text-text-secondary leading-relaxed">
                                Fenix xPad is running but the browser is blocking the connection.
                        ...
                        <div className="flex gap-3">
                            <button
                                onClick={() => window.open(fenixUrl, '_blank')}
                    ...
                    <>
                        {!iframeLoaded && (
                            <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 text-center z-10 bg-transparent">
                                ...
                                <p className="text-sm font-bold uppercase text-text-secondary">Loading Fenix xPad…</p>
                            </div>
                        )}
                        <iframe
                            src={fenixUrl}
            ...
            {showSettings && (
                <div className="shrink-0 mx-6 md:mx-8 mb-4 p-6 glass-panel z-20">
                    ...
                    <div className="flex items-center gap-3 max-w-lg">
                        <div className="flex-1 flex items-center bg-white/[0.03] border border-white/[0.05] rounded-xl overflow-hidden focus-within:border-accent-blue transition-colors">
                            ...
                            <input
                                ...
                                spellCheck={false}
                            />
                            <span className="pr-4 text-sm font-bold text-text-secondary select-none">:{FENIX_PORT}</span>
                        </div>
                        <button
                    ...
                    <p className="text-xs font-bold text-text-secondary/70 mt-3">
                        Enter the local network IP of the PC running MSFS + Fenix. Your IP is usually something like <span className="text-accent-blue">192.168.x.x</span>.
                    </p>
                </div>
            )}
            ...
            <div className="shrink-0 flex items-center justify-between px-6 md:px-8 py-4 border-t border-white/[0.05] bg-black/20 z-10">
                ...
                <div className="flex items-center gap-2 bg-black/20 p-1 border border-white/[0.05] rounded-xl">
                    ...
                    <button 
                        onClick={() => {
                            localStorage.setItem('xpad-last-efb', 'fslabs');
                            useUIStore.getState().setActiveModule('fslabs');
                        }}
